...
class bracketeer(object):
    ...
    def make_bracket(self, year, submissionPath, outputFilePath):
        ...
        def make_matchup_df(df, level, slot, previous_losers, dropna=False):
            ...
            slot: slot list for the above round (should be programmatically determined instead)
            ...
            out[LOWTEAM] = (pd.DataFrame([out['Team_x'], out['Team_y']])
                            .min()
                            .astype(int)
                            .astype(str))
            out[HIGHTEAM] = (pd.DataFrame([out['Team_x'], out['Team_y']])
                             .max()
                             .astype(int)
                             .astype(str))
            out = out[out[slot + '_x'] != out[slot + '_y']]
            df1 = out.loc[out['firstLower?'] == True, :]
            df1 = (df1.rename(columns={a: a.replace('x', LOWTEAM)
                   .replace('y', HIGHTEAM) for a in df1.columns}))
            df2 = out.loc[out['firstLower?'] == False, :]
            df2 = (df2.rename(columns={a: a.replace('x', HIGHTEAM)
                   .replace('y', LOWTEAM) for a in df2.columns}))
        ...
        def get_winners(df, level, matchupsdf, dropna=False):
            ...
            dropna: use for Roundof68 Only
            """
            df[ID] = (df[SEASON + '_' + LOWTEAM].astype(str) +
                      '_' + df[TEAM + '_' + LOWTEAM] + '_' +
                      df[TEAM + '_' + HIGHTEAM])
            windf = submit.set_index(ID).join(df.set_index(ID), how='inner')

            windf[WINNER] = ''
            windf.loc[windf[PRED] >= 0.5, WINNER] =\
                windf.loc[windf[PRED] >= 0.5, TEAM + '_' + LOWTEAM]
            windf.loc[windf[PRED] < 0.5, WINNER] =\
                windf.loc[windf[PRED] < 0.5, TEAM + '_' + HIGHTEAM]
            windf[LOSER] = ''
            windf.loc[windf[PRED] < 0.5, LOSER] =\
                windf.loc[windf[PRED] < 0.5, TEAM + '_' + LOWTEAM]
            windf.loc[windf[PRED] >= 0.5, LOSER] =\
                windf.loc[windf[PRED] >= 0.5, TEAM + '_' + HIGHTEAM]
            windf['round'] = level
        ...
        rounds = {'Roundof68': make_matchup_df(matchups,
                                               'Roundof68',
                                               'Slot68',
                                               pd.DataFrame(),
                                               dropna=True)}
        ...
        for r, s in zip(roundlist[1:], slotlist[1:]):
            ...
            losers = (pd.concat((losers, get_winners(rounds[r], r, matchups)[LOSER]))
        ...
        bracket[ID] = (
            bracket[SEASON + '_' + LOWTEAM].astype(str) +
            '_' + bracket[TEAM + '_' + LOWTEAM] + '_' +
            pd.DataFrame([bracket[WINNER], bracket[LOSER]])
            .max()
            .astype(int)
            .astype(str)
        )
        ...
        for slot, r in zip(slotlist, roundlist):
            bracket.loc[(bracket[ROUND] == r) & (bracket[WINNER] == bracket[LOWTEAM]), 'WSlot'] =\
                bracket.loc[(bracket[ROUND] == r) & (bracket[WINNER] == bracket[LOWTEAM]),
                            slot + '_' + LOWTEAM]
            bracket.loc[(bracket[ROUND] == r) & (bracket[WINNER] == bracket[HIGHTEAM]), 'WSlot'] =\
                bracket.loc[(bracket[ROUND] == r) & (bracket[WINNER] == bracket[HIGHTEAM]),
                            slot + '_' + HIGHTEAM]
            bracket.loc[(bracket[ROUND] == r) & (bracket[LOSER] == bracket[LOWTEAM]), 'LSlot'] =\
                bracket.loc[(bracket[ROUND] == r) & (bracket[LOSER] == bracket[LOWTEAM]),
                            slot + '_' + LOWTEAM]
            bracket.loc[(bracket[ROUND] == r) & (bracket[LOSER] == bracket[HIGHTEAM]), 'LSlot'] =\
                bracket.loc[(bracket[ROUND] == r) & (bracket[LOSER] == bracket[HIGHTEAM]),
                            slot + '_' + HIGHTEAM]

        bracket.loc[bracket[WINNER] == bracket[LOWTEAM], 'WSeed'] =\
            bracket.loc[bracket[WINNER] == bracket[LOWTEAM], 'Seed_' + LOWTEAM]
        bracket.loc[bracket[WINNER] == bracket[HIGHTEAM], 'WSeed'] =\
            bracket.loc[bracket[WINNER] == bracket[HIGHTEAM], 'Seed_' + HIGHTEAM]
        bracket.loc[bracket[LOSER] == bracket[LOWTEAM], 'LSeed'] =\
            bracket.loc[bracket[LOSER] == bracket[LOWTEAM], 'Seed_' + LOWTEAM]
        bracket.loc[bracket[LOSER] == bracket[HIGHTEAM], 'LSeed'] =\
            bracket.loc[bracket[LOSER] == bracket[HIGHTEAM], 'Seed_' + HIGHTEAM]

        bracket.loc[bracket[WINNER] == bracket[LOWTEAM], 'WPred'] =\
            bracket.loc[bracket[WINNER] == bracket[LOWTEAM], PRED]
        bracket.loc[bracket[WINNER] == bracket[HIGHTEAM], 'WPred'] =\
            1 - bracket.loc[bracket[WINNER] == bracket[HIGHTEAM], PRED]
        bracket.loc[bracket[WINNER] == bracket[LOWTEAM], 'LPred'] = None
        bracket.loc[bracket[WINNER] == bracket[HIGHTEAM], 'LPred'] = None

        slots1 = (bracket[['Loser_Name', 'LSlot', 'LSeed', 'LPred']]
                  .rename(columns={'LSlot': SLOT,
                                   'Loser_Name': 'Team_Name',
                                   'LSeed': 'Seed',
                                   'LPred': PRED}))
        slots2 = (bracket[['Winner_Name', 'WSlot', 'WSeed', 'WPred']]
                  .rename(columns={'WSlot': SLOT,
        ...
        for row in slots.values:
            if ~np.isnan(row[3]):
                slotsdict[int(row[1])] =\
                    row[2] + ' ' + row[0] + ' ' + "{0:.2f}%".format(row[3] * 100)
            else:
        ...
        if '%' in slotdata[134]:
            slotdata[135] = (matchup_locs[135],
                             ' '.join([a for a in slotsdict[134].split(' ')[:-1]]))
        else:
            slotdata[135] = (matchup_locs[135],
                             ' '.join([a for a in slotsdict[133].split(' ')[:-1]]))
